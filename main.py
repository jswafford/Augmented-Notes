...
class UploadHandler(blobstore_handlers.BlobstoreUploadHandler):
  ...
  def post(self):
    ...
    urls = {}
    for key in keys:
      infos = self.get_uploads(key)
      if infos:
        urls[key] = "/serve/{0}".format(infos[0].key())
      else:
        urls[key] = defaults[key]
        # self.redirect("/?empty=1")
    # default_page_urls = ['/static/tmp/1.jpg', '/static/tmp/2.jpg',]
    default_page_urls = []
    for i in range(7):
      default_page_urls.append('/static/tmp/{0}.jpg'.format(i+1))
    page_infos = self.get_uploads("page")
    if page_infos:
      urls['pages'] = []
      for page_info in page_infos:
        urls['pages'].append("/serve/{0}".format(page_info.key()))
    else:
      urls['pages'] = default_page_urls
